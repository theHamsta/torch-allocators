...
struct HostAllocator {
  ...
  cudaError_t malloc(void **ptr, size_t size) {
    ...
    // at::detail::getCUDAHooks().getDevceIndexWithPrimaryContext(); if
    // (primary_ctx_device_index.has_value()) {
    // device_guard.reset_device(at::Device(at::DeviceType::CUDA,
    // *primary_ctx_device_index));
    //}
...
cudaError_t THCCachingHostAllocator_recordEvent(void *ptr,
                                                at::cuda::CUDAStream stream) {
  return allocator.recordEvent(ptr, stream);
}

void THCCachingHostAllocator_emptyCache() { allocator.emptyCache(); }

static void THCCachingHostDeleter(void *ptr) { allocator.free(ptr); }
